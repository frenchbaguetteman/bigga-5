...
void Drive::raw_pid_odom_pp_set(std::vector<odom> imovements, bool slew_on) {
  ...
  boomerang_ptf_side_initialized = false;
  boomerang_log_counter = 0;
  boomerang_log_index = -1;
...
void Drive::raw_pid_odom_ptp_set(odom imovement, bool slew_on) {
  ...
  bool is_current_boomerang = imovement.target.theta != ANGLE_NOT_SET;

  // Calculate the point to look at
  bool set_global_ptf = true;
  if (is_current_boomerang) {
    set_global_ptf = !boomerang_ptf_side_initialized;
    boomerang_ptf_side_initialized = true;
  } else {
    boomerang_ptf_side_initialized = false;
  }
  point_to_face = find_point_to_face(odom_pose_get(), {imovement.target.x, imovement.target.y}, current_drive_direction, set_global_ptf);
  ...
  if (print_toggle && is_current_boomerang && boomerang_log_index != pp_index) {
    pose current = odom_pose_get();
    const char* dir_name = current_drive_direction == REV ? "rev" : "fwd";
    printf("  Boomerang Config: dir=%s start:(%.2f, %.2f, %.2f) target:(%.2f, %.2f, %.2f) lookahead:%.2f dlead:%.3f max_carrot:%.2f max_speed:%d\n",
           dir_name,
           current.x,
           current.y,
           current.theta,
